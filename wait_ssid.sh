...
if ! tmuc has-session -t momo 2>/dev/null; then
	tmux new-session -d -s momo "gst-launch-1.0 v4l2src device=/dev/video0 io-mode=2 ! image/jpeg,width=1280,height=720,framerate=30/1 ! jpegdec ! videoconvert ! openh264enc bitrate=6000000 complexity=0 ! rtph264pay pt=96 ! udpsink host=192.168.11.2 port=5000 sync=false
"
fi
